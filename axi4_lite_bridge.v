...
module axi4_lite_bridge #(
    ...
    // ----------------------------------------------------------------
    // ----------------------------------------------------------------
    // FSM chính của Cầu nối AXI Bridge (Đã sửa lỗi Timing Collision)
    // ----------------------------------------------------------------
            ...
            case (state)
                ...
                BR_WR_R: begin
                    if (m_bvalid && m_bready) begin
                        ...
                        state     <= BR_DONE; // Đã sửa: Chuyển về BR_DONE thay vì BR_IDLE
                    end
                end
                ...
                // ---- Đã sửa: Trạng thái BR_DONE chờ đồng bộ CPU ----
                BR_DONE: begin
                    mem_ready <= 1'b0;
                    state     <= BR_IDLE;
                end
